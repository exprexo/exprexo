...
test('router: a Router instance', (t) => {
  ...
  const testMethod = method => t.ok(actualMethods.find(m => m === method),
...
test('router(middleware): `options` method should return `200`',
  (t) => {
    ...
    const testHandler = (handler) => {
      ...
      t.ok(responseSpies.status.withArgs(200).calledOnce,
        `${handler.method} should always return 200`)
      t.ok(responseSpies.json.withArgs({}).calledOnce,
